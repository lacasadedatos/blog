...
description: |
  Si una imagen vale más que mil palabras... ¿cuánto vale una imagen hecha de palabras?... La minería de texto nos permite realizar estudios sociales partiendo de grandes volúmenes de textos. Partiendo  de miles de publicaciones en redes sociales, podemos identificar los temas de discusión asociados a un tema concreto, los sentimeintos asociados a ellos y su evolución en el tiempo.
author:
...
trump <- read_rds("trump2k.rds") %>% mutate(id = as.character(id)) %>% 
...
#| echo: FALSE
set.seed(1976)
gen_wordGraph2(toots_Positive, 50, 0.3)+
...
#| echo: FALSE
set.seed(1976)
gen_wordGraph2(toots_Negative, 15, 0.2)+
  ggtitle("C) Negative toots - Cluster 3")
```
...
APretti_trust %>% 
  ...
  flextable::flextable() %>% 
  add_header_lines("Tabla 5. Adjetivos Relacionados con la confianza en el toot 115957194868532631") %>%
...
Epstein_trust %>% 
  ...
  setNames(c("palabra", "sentimiento")) %>% 
  flextable::flextable() %>% 
  add_header_lines("Tabla 6. Adjetivos Relacionados con la confianza en el toot 116036946032124872") %>%
  fontsize(part= "all", size = 10) %>% 
  theme_zebra() %>%
  flextable::align(align = "left",
                   part = c("all")) %>% 
